...
namespace LightweightMatrixCSharp {
    public class Matrix
    {
        ...
        public Matrix(int iRows, int iCols)         // Matrix Class constructork
        {
            rows = iRows;
            cols = iCols;
        ...
        public Matrix(float[,] matrix, bool rowMajor = true)
        {
            if (rowMajor) {
                rows = matrix.GetLength(0);
                cols = matrix.GetLength(1);
                mat = matrix.Cast<float>().ToArray();  // NOTE copies elements
            } else {  // column-major
        ...
        public void MakeRref()                                    // Function makes reduced echolon form
        {
            ...
            for (int r = 0; r < rows; r++)
            {
                ...
                float div = this[r, lead];
                for (int j = 0; j < cols; j++) this[r, j] /= div;
                for (int j = 0; j < rows; j++)
                {
                    if (j != r)
                    {
                        float sub = this[j, lead];
        ...
        public static Matrix RandomMatrix(int iRows, int iCols, float dispersion)       // Function generates the random matrix
        {
            ...
            for (int i = 0; i < iRows; i++)
                for (int j = 0; j < iCols; j++)
                    matrix[i, j] = (float)random.NextDouble() * (dispersion - -dispersion) - dispersion;
            return matrix;
        }
